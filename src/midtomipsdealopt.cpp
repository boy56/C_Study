...
namespace compiler {
	...
	std::string idToReg(int num) {
		if ((num < 0) || (num > 17)) {
			std::cout << "There is a bug in midtomipsdeal--idToReg, reg_num out of range" << std::endl;
			return "error";
		}
	...
	*/
	...
	void varCount(std::string name, std::map<std::string, int>& s_count_map) {
		if (string_is_num(name) || (name[0] == 'T')) return;	//不是局部变量
		else {
			if (find_in_tab(name) == 0) {
				std::cout << "There is a bug in midtomipsdealopt--varCount: name not find in tab" << std::endl;
			}
			else {
	...
	void funcAllocRegs(int func_start) {
		...
		for (i = func_start + 1; i < func_end; i++) {
			...
			if ((t.op >= 0) && (t.op < 4)) {	//+ - * /
				varCount(t.z, s_Count_Map);
				varCount(t.x, s_Count_Map);
				varCount(t.y, s_Count_Map);
			}
			else if (t.op == 4) {	// 赋值
				if (t.x == " ") {	//普通赋值
					varCount(t.z, s_Count_Map);
	...
	*/ 
	...
	//标签的处理
	void setlabDeal(midcode cur_mid, int mid_pos) {
	...
	//函数声明的处理
	void funcDeal(midcode cur_mid, int mid_pos) {
		int t_pos, b_pos;
		int func_start = mid_pos;
		t_pos = find_in_tab(cur_mid.y);
		b_pos = tab.tabArray[t_pos].ref;
