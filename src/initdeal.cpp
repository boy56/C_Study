...
namespace compiler{
	...
	void openFile(){
		...
		std::cin >> inFileName;
		/*
		//判断文件名是否和"output.txt","result.txt"重名
		while (inFileName == "output.txt" || inFileName == "result.txt") {
			std::cout << "Error:source-code_file_name is illegal, please input again(don't be \"output.txt\" or \"result.txt\")" << std::endl;
			std::cin >> inFileName;
		}
		*/
		...
		erroroutFile.open("result/errorinfo.txt",std::fstream::out);
		erroroutFile << "*******************************" << std::endl;
		word_out_file.open("some_info_out/word_output.txt", std::fstream::out);
